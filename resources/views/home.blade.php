                        ...
                        @auth
                            <div class="w-full text-center mt-4">
                                ...
                                <div class="mt-4 flex flex-col gap-3">
                                    <a href="{{ route('cart') }}" class="w-full text-center rounded-lg h-12 flex items-center justify-center px-4 bg-gray-200 dark:bg-gray-700 text-sm font-bold hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">Hồ sơ cá nhân</a>
                                    <a href="{{ route('profile') }}" class="w-full text-center rounded-lg h-12 flex items-center justify-center px-4 bg-gray-200 dark:bg-gray-700 text-sm font-bold hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">Giỏ hàng</a>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // 🟦 Nút Mua vé
        muaVeBtn.addEventListener("click", async function () {
        const response = await fetch('/booking/store', {
            method: 'POST',
            headers: {'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken},
            body: JSON.stringify({
                phim_id: phimSelect.value,
                rap_id: rapSelect.value,
                ngay_chieu: ngayInput.value,
                gio_chieu: gioSelect.value
            })
        });

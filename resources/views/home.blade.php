    ...
    <div class="flex min-h-screen w-full flex-col">
        ...
        <main>
            <section class="relative group">
                <div id="banner-slider" class="relative w-full h-56 sm:h-72 md:h-96 bg-cover bg-center">
                    
                    <button id="prev-slide" class="absolute top-1/2 left-4 z-20 -translate-y-1/2 rounded-full bg-white/20 p-2 text-white backdrop-blur-sm transition opacity-0 group-hover:opacity-100 hover:bg-white/30">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
                    </button>
                    <button id="next-slide" class="absolute top-1/2 right-4 z-20 -translate-y-1/2 rounded-full bg-white/20 p-2 text-white backdrop-blur-sm transition opacity-0 group-hover:opacity-100 hover:bg-white/30">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
                    </button>

                    <div class="absolute bottom-0 left-0 w-full h-full">
                        <div id="banner-content" class="absolute bottom-0 p-6 md:p-8 z-10 container mx-auto">
                            <h2 id="banner-title" class="text-white text-2xl md:text-4xl font-bold [text-shadow:0_2px_4px_rgba(0,0,0,0.7)]"></h2>
                            <p id="banner-description" class="text-white/80 mt-2 max-w-lg [text-shadow:0_1px_3px_rgba(0,0,0,0.7)]"></p>
                            <div class="mt-4 flex gap-4">
                                <button class="min-w-[120px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-12 px-6 bg-primary text-white text-sm font-bold hover:bg-primary/90 transition-colors btn-glow">Buy Tickets</button>
                                <button class="min-w-[120px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-12 px-6 bg-white/20 text-white text-sm font-bold hover:bg-white/30 backdrop-blur-sm transition-colors">Details</button>
                            </div>
                        </div>
                    </div>
                    <div id="banner-dots" class="absolute bottom-0 w-full flex justify-center gap-2 p-4"></div>
                </div>
            </section>
            ...
            <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div class="border-b border-gray-800 mb-6">
                    <nav class="flex space-x-8 text-sm">
                        <button data-tab="phim-dang-chieu" class="tab-btn pb-2 border-b-2 text-primary border-primary font-semibold transition-colors duration-200">
                            Movies Showing
                        </button>
                        <button data-tab="phim-sap-chieu" class="tab-btn pb-2 border-b-2 text-gray-400 border-transparent font-semibold transition-colors duration-200">
                            Upcoming Movies
                        </button>
                    </nav>
                <div id="phim-dang-chieu" class="tab-content mt-5 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-x-6 gap-y-8">
@foreach($phimDangChieu as $phim)
    <div class="flex flex-col h-full">
        <div class="atropos rounded-lg bg-gray-800/50 shadow-lg flex-grow">
            <div class="atropos-scale">
                <div class="atropos-rotate">
                    <div class="atropos-inner rounded-lg overflow-hidden">
                        
                        <div class="relative group cursor-pointer aspect-[2/3]">
                            <img data-atropos-offset="-5" alt="{{ $phim->ten_phim }}" 
                                 class="h-full w-full object-cover" 
                                 src="{{ asset('storage/' . $phim->anh_poster) }}">
                            
                            <div data-atropos-offset="0" class="absolute inset-0 flex flex-col items-center justify-start bg-slate-900/80 p-2 text-center text-white opacity-0 transition-opacity duration-500 group-hover:opacity-100 sm:p-4">
                                <h3 class="mb-2 font-bold text-base line-clamp-2 sm:mb-3 sm:text-lg">{{ $phim->ten_phim }}</h3>
                                <ul class="mb-3 inline-block space-y-2 text-left sm:mb-4 sm:space-y-4">
                                    <li class="flex items-center gap-2">
                                        <i class="fa-solid fa-users w-4 text-yellow-400"></i>
                                        <span class="text-[11px] sm:text-xs">{{ $phim->the_loai ?? 'Đang cập nhật' }}</span>
                                    </li>
                                    <li class="flex items-center gap-2">
                                        <i class="fa-solid fa-clock w-4 text-yellow-400"></i>
                                        <span class="text-[11px] sm:text-xs">{{ $phim->thoi_luong ?? 'N/A' }} phút</span>
                                    </li>
                                    <li class="flex items-center gap-2">
                                        <i class="fa-solid fa-globe-asia w-4 text-yellow-400"></i>
                                        <span class="text-[11px] sm:text-xs">{{ $phim->quoc_gia ?? 'N/A' }}</span>
                                    </li>
                                    <li class="flex items-center gap-2">
                                        <i class="fa-solid fa-comment-dots w-4 text-yellow-400"></i>
                                        <span class="text-[11px] sm:text-xs">{{ $phim->ngon_ngu ?? 'N/A' }}</span>
                                    </li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div class="p-3 text-center">
                            <p data-atropos-offset="2" class="text-xs text-gray-400">
                                Khởi chiếu: {{ $phim->ngay_khoi_chieu ? \Carbon\Carbon::parse($phim->ngay_khoi_chieu)->format('d/m/Y') : 'N/A' }}
                            </p>
                            <h3 data-atropos-offset="4" class="mt-1 font-bold text-white text-base leading-tight">
        ...
        <div class="mt-3 flex items-center justify-center gap-4">
            <button class="trailer-btn flex items-center gap-2 text-sm text-gray-300 hover:text-white transition-colors" data-trailer-id="{{ $phim->trailer_id ?? '' }}">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-yellow-400"><circle cx="12" cy="12" r="10"></circle><polygon points="10 8 16 12 10 16 10 8"></polygon></svg>
                <span class="underline">Xem Trailer</span>
            </button>
            <a href="#" class="rounded bg-yellow-400 px-4 py-2 text-xs font-bold text-black transition-colors hover:bg-yellow-500">
                TÌM HIỂU THÊM
            </a>
        </div>
    </div>
@endforeach
                ...
                <div id="phim-sap-chieu" class="tab-content mt-5 hidden grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-x-6 gap-y-8">
                    @foreach($phimSapChieu as $phim)
                        <div>
                            <div class="atropos rounded-lg bg-gray-800/50 shadow-lg">
                                <div class="atropos-scale">
                                    <div class="atropos-rotate">
                                        <div class="atropos-inner rounded-lg overflow-hidden">
                                            <div class="relative group cursor-pointer">
                                                <img data-atropos-offset="-5" alt="{{ $phim->ten_phim }}" class="h-full w-full object-cover" src="{{ asset('storage/' . $phim->anh_poster) }}">
                                                <div data-atropos-offset="0" class="absolute inset-0 flex flex-col items-center justify-start bg-slate-900/80 p-2 text-center text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100 sm:p-4">
                                                    <h3 class="mb-2 font-bold text-base line-clamp-2 sm:mb-3 sm:text-lg">{{ $phim->ten_phim }}</h3>
                                                    <ul class="mb-3 inline-block space-y-2 text-left sm:mb-4 sm:space-y-4">
                                                        <li class="flex items-center gap-2">
                                                            <i class="fa-solid fa-users w-4 text-yellow-400"></i>
                                            ...
                                            <div class="p-3 text-center">
                                                <p data-atropos-offset="2" class="text-xs text-gray-400">Khởi chiếu: {{ $phim->ngay_khoi_chieu ? \Carbon\Carbon::parse($phim->ngay_khoi_chieu)->format('d/m/Y') : 'N/A' }}</p>
                                                <h3 data-atropos-offset="4" class="mt-1 font-bold text-white text-base leading-tight"><a href="#" class="hover:text-yellow-400 transition-colors">{{ $phim->ten_phim }}</a></h3>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="mt-3 flex items-center justify-center gap-4">
                                ...
                                <a href="#" class="rounded bg-yellow-400 px-4 py-2 text-xs font-bold text-black transition-colors hover:bg-yellow-500">TÌM HIỂU THÊM</a>
                            </div>
                        </div>
                    @endforeach
                </div>
          ...
          <section class="mb-12 fade-in-section">
              <div class="bg-gradient-to-r from-primary via-red-600 to-red-800 dark:from-primary dark:to-red-900 rounded-xl p-8 flex flex-col md:flex-row items-center justify-between gap-6 animated-gradient">
                  ...
                  <button class="flex-shrink-0 min-w-[150px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-12 px-6 bg-white text-primary text-sm font-bold hover:bg-gray-200 transition-colors">Sign Up Now</button>
              </div>
          </section>
          @endguest
                ...
                <section class="mb-12 fade-in-section">
                    <h2 class="text-2xl font-bold mb-6 text-gray-900 dark:text-white">Special Formats</h2>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                        <div class="rounded-lg overflow-hidden group relative shadow-lg"><img alt="IMAX" class="w-full aspect-[16/9] object-cover rounded-lg transition-transform duration-300 group-hover:scale-105" src="https://placehold.co/600x400/111827/ffffff?text=IMAX" /></div>
                        <div class="rounded-lg overflow-hidden group relative shadow-lg"><img alt="4DX" class="w-full aspect-[16/9] object-cover rounded-lg transition-transform duration-300 group-hover:scale-105" src="https://placehold.co/600x400/003366/ffffff?text=4DX" /></div>
                        <div class="rounded-lg overflow-hidden group relative shadow-lg"><img alt="Gold Class" class="w-full aspect-[16/9] object-cover rounded-lg transition-transform duration-300 group-hover:scale-105" src="https://placehold.co/600x400/a67c00/ffffff?text=GOLD+CLASS" /></div>
                        <div class="rounded-lg overflow-hidden group relative shadow-lg"><img alt="ScreenX" class="w-full aspect-[16/9] object-cover rounded-lg transition-transform duration-300 group-hover:scale-105" src="https://placehold.co/600x400/3d3d3d/ffffff?text=ScreenX" /></div>
                    </div>
                </section>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            // --- BANNER SLIDER ---
            const slides = [
                { title: "AVATAR: THE TULKUN RIDER", description: "Return to Pandora and explore the uncharted oceans with a new epic adventure.", image: "https://image.tmdb.org/t/p/original/VlSrZXo_f0g0i9b8i2gQZARb0vF.jpg" },
                { title: "THE BATMAN: PART II", description: "The Riddler's game is over, but a new threat emerges from the shadows of Gotham.", image: "https://image.tmdb.org/t/p/original/5i6SjyDbDWqyun8klUuCxrlFbyw.jpg" },
                { title: "MINECRAFT: THE MOVIE", description: "A live-action adventure based on the world's most popular game. Build your destiny.", image: "https://image.tmdb.org/t/p/original/sQLC8v_i_dMkuGeZbk1S32g9gGg.jpg" },
                { title: "ZOOTOPIA 2", description: "Judy Hopps and Nick Wilde return for another thrilling case in the bustling metropolis.", image: "https://image.tmdb.org/t/p/original/8gLh2P3Dax2MzaL58S06H237l3g.jpg" }
            ];
            let currentSlide = 0;
            ...
            function showSlide(index) {
                ...
                banner.style.backgroundImage = `linear-gradient(90deg, rgba(18, 18, 18, 0.8) 0%, rgba(18, 18, 18, 0.6) 25%, rgba(18, 18, 18, 0) 50%), url("${slide.image}")`;
            ...
            // ========== NEW: User Menu Dropdown Handler ==========
            ...
            const initAtropos = () => {
                ...
                if (atroposElements.length === 0) return;
            ...
            tabButtons.forEach(button => {
                button.addEventListener('click', () => {
                    ...
                    setTimeout(initAtropos, 50); // Khởi tạo lại Atropos cho tab mới hiển thị
                });
            });
        ...
        movieCards.forEach(card => {
            card.addEventListener('click', function(event) {
                // Ngăn không cho sự kiện click lan ra ngoài document
                event.stopPropagation(); 
                
                const currentOverlay = this.querySelector('.movie-overlay');
                const isActive = currentOverlay.classList.contains('is-active');

                // Trước tiên, đóng tất cả các overlay khác
                document.querySelectorAll('.movie-overlay').forEach(overlay => {
                    overlay.classList.remove('is-active');
                });

                // Nếu overlay vừa click chưa active, thì mở nó ra
                if (!isActive) {
                    currentOverlay.classList.add('is-active');
                }
                // Nếu đã active rồi thì thôi, bước trên đã đóng nó lại
            });
        });

        // Thêm sự kiện click vào toàn bộ trang để đóng overlay
        document.addEventListener('click', function() {
            document.querySelectorAll('.movie-overlay.is-active').forEach(overlay => {
                overlay.classList.remove('is-active');
            });
        });

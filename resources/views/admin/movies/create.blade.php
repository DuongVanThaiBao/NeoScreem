...
<head>
    ...
    <title>Thêm phim mới - NeoScreem</title>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
            <div class="flex items-center space-x-3">
                ...
                <span class="text-xl font-bold text-white">Thêm phim mới</span>
            </div>
...
<div class="layout">
    <!-- Sidebar (copy from dashboard for consistency) -->
    <aside class="sidebar bg-dark-surface border-r border-dark-border flex flex-col">
        ...
        <nav class="flex-1 p-4">
            ...
            <div class="mb-6">
                ...
                <ul class="space-y-2">
                    <li><a href="/admin/hr/employees" class="sidebar-item flex items-center space-x-3 px-4 py-3 rounded-lg text-gray-300 hover:text-white hover:bg-retail-green/10 transition-colors"><i class="fas fa-users text-gray-400"></i><span>Quản lý nhân sự</span></a></li>
                    <li><a href="/admin/hr/schedules" class="sidebar-item flex items-center space-x-3 px-4 py-3 rounded-lg text-gray-300 hover:text-white hover:bg-retail-green/10 transition-colors"><i class="fas fa-calendar-check text-gray-400"></i><span>Lịch làm việc</span></a></li>
                    <li><a href="/admin/hr/reports" class="sidebar-item flex items-center space-x-3 px-4 py-3 rounded-lg text-gray-300 hover:text-white hover:bg-retail-green/10 transition-colors"><i class="fas fa-chart-bar text-gray-400"></i><span>Báo cáo nhân sự</span></a></li>
                </ul>
            </div>
            <div class="mb-6">
    ...
    <main class="main-content p-6 space-y-6">
        <div class="card">
            <h3 class="text-lg font-semibold text-white mb-4">Thông tin phim</h3>
            @if ($errors->any())
            ...
            <form method="POST" action="{{ route('admin.movies.store') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="title" name="title" class="w-full mt-1 px-3 py-2 border rounded" value="{{ old('title') }}" required>
                    </div>
                    <div>
                        <label for="genre">Thể loại</label>
                        <input type="text" id="genre" name="genre" class="w-full mt-1 px-3 py-2 border rounded" value="{{ old('genre') }}" required>
                    </div>
                    <div>
                        <label for="director">Đạo diễn</label>
                        <input type="text" id="director" name="director" class="w-full mt-1 px-3 py-2 border rounded" value="{{ old('director') }}" required>
                    </div>
                    <div>
                        <label for="cast">Diễn viên</label>
                        <input type="text" id="cast" name="cast" class="w-full mt-1 px-3 py-2 border rounded" value="{{ old('cast') }}" required>
                    </div>
                    <div>
                        <label for="duration">Thời lượng (phút)</label>
                        <input type="number" id="duration" name="duration" class="w-full mt-1 px-3 py-2 border rounded" value="{{ old('duration') }}" min="1" required>
                    </div>
                    <div>
                        <label for="release_date">Ngày phát hành</label>
                        <input type="date" id="release_date" name="release_date" class="w-full mt-1 px-3 py-2 border rounded" value="{{ old('release_date') }}" required>
                    </div>
                    <div>
                        <label for="status">Trạng thái</label>
                        <select id="status" name="status" class="w-full mt-1 px-3 py-2 border rounded" required>
                            <option value="đang chiếu" @selected(old('status')==='đang chiếu')>Đang chiếu</option>
                            <option value="sắp chiếu" @selected(old('status')==='sắp chiếu')>Sắp chiếu</option>
                            <option value="đã kết thúc" @selected(old('status')==='đã kết thúc')>Đã kết thúc</option>
                        </select>
                    </div>
                    <div>
                        <label for="language">Ngôn ngữ</label>
                        <input type="text" id="language" name="language" class="w-full mt-1 px-3 py-2 border rounded" value="{{ old('language') }}" required>
                    </div>
                    <div>
                        <label for="country">Quốc gia</label>
                        <input type="text" id="country" name="country" class="w-full mt-1 px-3 py-2 border rounded" value="{{ old('country') }}" required>
                    </div>
                    <div class="md:col-span-2">
                        <label for="description">Mô tả</label>
                        <textarea id="description" name="description" rows="4" class="w-full mt-1 px-3 py-2 border rounded" required>{{ old('description') }}</textarea>
                    </div>
                    <div>
                        <label for="poster">Poster (URL)</label>
                        <input type="text" id="poster" name="poster" class="w-full mt-1 px-3 py-2 border rounded" value="{{ old('poster') }}">
                    </div>
                    <div>
                        <label for="trailer">Trailer (URL)</label>
                        <input type="text" id="trailer" name="trailer" class="w-full mt-1 px-3 py-2 border rounded" value="{{ old('trailer') }}">
                    </div>
                </div>
                <div class="pt-2">

...
<div class="bg-gray-900 text-white min-h-screen">
    <div class="max-w-6xl mx-auto py-12 px-6">
        ...
        <h2 class="text-2xl font-semibold mb-4">Các rạp đang chiếu phim</h2>

        @php
            $raps = $phim->showtimes->map(fn($st) => $st->room->theater)->unique('id');
        @endphp

        @foreach($raps as $rap)
            <div class="bg-gray-800 p-4 rounded-lg mb-4">
                <h3 class="text-xl font-semibold">{{ $rap->name }}</h3>
                <p class="text-gray-400">{{ $rap->location }}</p>

                {{-- Suất chiếu --}}
                @foreach($phim->showtimes->where('room.theater.id', $rap->id) as $showtime)
                    <div class="mt-2 p-2 bg-gray-700 rounded-md">
                        ...
                        {{-- Form chọn ghế + snack cho từng suất --}}
...
document.querySelectorAll('.booking-form').forEach(form => {
    ...
    /**
     * Hàm tính toán và cập nhật tổng tiền cho form này
     */
    function updateTotal() {
        ...
        seats.forEach(cb => {
            if (cb.checked) {
                // Lấy giá từ data-attribute của checkbox
        ...
        snacks.forEach(snack => {
            ...
            if (qty > 0) {
                // Lấy giá từ data-attribute của input snack
    ...
    // --- PHẦN ĐÃ SỬA ---
    // Lắng nghe sự kiện 'change' (thay đổi) trên checkbox
    seats.forEach(cb => {
        const seatDiv = cb.nextElementSibling; // div hiển thị ghế

        if (!cb.disabled) {
            cb.addEventListener('change', () => {
                // Khi click vào label (div), checkbox sẽ tự động thay đổi
                // Chúng ta chỉ cần dựa vào trạng thái 'checked' mới của nó
                // để đổi màu và tính lại tổng tiền.
